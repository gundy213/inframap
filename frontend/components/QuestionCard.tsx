...
export const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-xl p-8 border border-gray-100 ${className}`}>
      {question.category && (
        <div className="mb-6">
          <span className="inline-block bg-gradient-to-r from-purple-100 to-blue-100 text-purple-800 text-xs font-bold px-4 py-2 rounded-full shadow-sm">
            {question.category}
          </span>
        </div>
      )}
      ...
      <div className="space-y-4">
        {question.options.map((option) => (
          <div
            key={option.id}
            className={`border-2 rounded-xl p-5 cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-105 ${
